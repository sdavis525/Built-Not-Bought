...
const MoreWorkouts = ({ targetMuscleWorkouts, equipmentWorkouts }) => (
  <Box sx={{ mt: { lg: '100px', xs: '0px' } }}>
    <Typography sx={{ fontSize: { lg: '44px', xs: '25px' }, ml: '20px' }} fontWeight={700} color="#000" mb="33px">
      Similar <span style={{ color: '#FF2625', textTransform: 'capitalize' }}>Target Muscle</span> workouts
    </Typography>
    <Stack direction="row" sx={{ p: 2, position: 'relative' }}>
      {targetMuscleWorkouts.length !== 0 ? <HorizontalScrollbar data={targetMuscleWorkouts} /> : <Loader />}
    </Stack>
    <Typography sx={{ fontSize: { lg: '44px', xs: '25px' }, ml: '20px', mt: { lg: '100px', xs: '60px' } }} fontWeight={700} color="#000" mb="33px">
      More <span style={{ color: '#FF2625', textTransform: 'capitalize' }}>Equipment</span> workouts
    </Typography>
    <Stack direction="row" sx={{ p: 2, position: 'relative' }}>
